%SVO Support Vector Optimiser, low-level routine
%
...
%                       as an optimisation failure and treated accordingly (deafault: 1)
%    .PF_ON_FAILURE     if optimisation is failed (or bias is undefined and BIAS_IN_ADMREG is 0)
...
function [v,J,C,nu] = svo(K,y,C,Options)
  ...
  Options = updstruct(DefOptions, Options,1);
	...
	if nargin < 3 | isempty(C)
		prwarning(3,'The regularisation parameter C is not specified, assuming 1.');
    C = 1;
	end
  ...
  D = (D+D')/2; % guarantee symmetry
	...
	% 'qp' minimises:   0.5 x' K x + f' x
	% subject to:       Ax <= b
	%
	if (exist('qld') == 3)
		v = qld (D, f, -A, b, lb, ub, p, 1);
	elseif (exist('quadprog') == 2)
		prwarning(2,'QLD not found, the Matlab routine QUADPROG is used instead.')
		opt = optimset; opt.LargeScale='off'; opt.Display='off';
		v = quadprog(D, f, [], [], A, b, lb, ub,[],opt);
	else 
		prwarning(2,'QLD not found, the Matlab routine QP is used instead.')
		verbos = 0;
		negdef = 0;
		normalize = 1;
  ...
  Goutry
   ...
   % check if we don't have a zero-weight vector
   if var(K*v)<1e-5
      prwarning(3,'Output of the SVM is (almost) constant. Please increase C or rescale your data.');
   end
   ...
   % Find the SV on the boundary
    ...
    elseif Options.bias_in_admreg
      ...
      prwarning(3,['The bias term is undefined. The midpoint of its admissible region is used.']);    
      b = (lb+ub)/2;
    ...
    if Options.pf_on_failure
      prwarning(2,[lasterr ' Pseudo-Fisher is computed instead.']);
      n = size(K,1);
